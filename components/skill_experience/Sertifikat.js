      ...
      {SertifikatContent?.sertifikat.map((sertif, idx) => {
        return (
          <div className='relative mt-4' key={idx}>
            <div className='flex flex-col justify-between w-full h-20 px-2 py-2 border shadow-md rounded-xl hover:opacity-5 '>
              <h3 className='font-semibold text-black-figma'>{sertif?.title}</h3>
              <p className='text-sm text-gray-500'>{sertif?.tanggal}</p>
            </div>
            <div className='absolute top-0 w-full h-20 bg-gray-500 opacity-0 rounded-xl hover:opacity-90 hover:block'>
